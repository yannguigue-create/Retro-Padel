...
def afficher_classement():
    ...
    # Arrondit et formate les points à une décimale pour l'affichage
    df["Points"] = df["Points"].map('{:.1f}'.format)
    
    df = df[["Rang", "Joueur", "Sexe", "Points", "Jeux", "Matchs"]]
    
    # Convertit en HTML pour appliquer la classe CSS et le rendre compact
    st.markdown(df.to_html(index=False, classes="small-table"), unsafe_allow_html=True)
